<template>
  <main class="py-8 px-4 flex flex-col">
    <h1 class="inter-medium font-color-green font-size-24 page-title">Your reservations</h1>
    ...
    <div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mt-6 mb-4">
        ...
        <div v-else>
          <CardReservations v-for="(card, index) in thisWeek" :key="index" :guest="card.guest" :property="card.property"
    ...
    <div>
      ...
      <hr />
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mt-6 mb-4">
        <CardReservations v-for="(card, index) in futureWeeks" :key="index" :guest="card.guest"
          :property="card.property" :startDate="card.startDate" :endDate="card.endDate" />
      </div>
    </div>
    ...
    <div>
      ...
      <hr />
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mt-6 mb-4">
        <CardReservations v-for="(card, index) in previousWeeks" :key="index" :guest="card.guest"
          :property="card.property" :startDate="card.startDate" :endDate="card.endDate" />
      </div>
    </div>
  </main>
